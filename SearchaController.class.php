...
class SearchaController extends BaseController {
...
//            $result = M('')->query($sql);
            ...
            $show = $Page->show();
            $result = M('Company')->where($map)->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
...
//      );
     ...
     public function companyDetails(){
        ...
        if ( $id ) {
            ...
            $data = [
                ...
                'pageSize'=>10,
                'resultTypeSelect'=>1,
                'searchExpression'=>"申请（专利权）人=(".$company['Name'].")",
            ...
            if($result['pageResult']['total'] > 0){
                $patent_count = $result['pageResult']['total'];
            }else{
                $patent_count = 0;
            }

            $company['patent_count'] = $patent_count;
            ...
            $patent =$result['pageResult']['records'];
  ...
  //   }
    ...
    public function objectList(){
        ...
        if (IS_GET) {
           ...
           //  }
            ...
            if (!session('user_id')) {
               $data = $model->where($map)->order("id desc")->limit(5)->select();
               $pinggu = 1;
               $this->assign($pinggu);
            }else{
    ...
    public function ajaxObjectDetails(){
        ...
        $this->assign('funding',$object['publicity']);
        $this->assign('funds',$object['publicity'][0]['funding']);
        $this->display();
         // dump($object);
        //}
    }
...
//        $this->assign('goodscategory',$goodscategory);
    ...
    public function ruanzhu(){
        ...
        $this->assign('backurl',U('Index/index'));
        $this->assign('title','软著搜索');
        $this->assign('right','<span id="submit-button">搜索</span>');
    ...
    public function patent(){
        ...
        if(!empty($keyword) && $hash=="guanwangrenzheng"){
            ...
            $count= M('patent3')->where($map)->count();
            $page = new Page($count,7);
            $result = M('patent3')->where($map)->limit($page->firstRow.','.$page->listRows)->select();
            $this->assign('page',$page->show());
            $this->assign('result',$result);
    ...
    public function searchPatent2(){
        ...
        if(!empty($keyword)){
            ...
            /*$url = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$keyword.'&pageIndex='.$p;
            //echo $url;
            $res = json_decode(file_get_contents($url),true);
            //var_dump($res);
            if($res['Paging']['TotalRecords'] > 0){
                $this->assign('result',$res['Result']);
                $page = new \Think\Page($res['Paging']['TotalRecords'],10);
                $this->assign('page',$page->show());
                $this->assign('count',$res['Paging']['TotalRecords']);
            }*/

//            $url = 'http://120.78.133.212:8000/get_patent/?keyword='.urlencode($keyword).'&t='.$type.'&p='.$p;
//
//
//            $result = json_decode(file_get_contents($url),true);
//            $count = $result['cubePatentSearchResponse']['total_hits'];
//
//
...
/*        $this->assign('history', $this->unset_keyword(M('SearchRecord')->order(array('create_time' => 'desc'))->where(array('uid' => session('user.user_id'), 'type' => 2))->limit(10)->select()));
    ...
    public function patentList(){
        $company = M('Company')->find(I('get.id'));
//        $url1 = 'http://120.78.133.212:8000/get_patent_count/?keyword='.urlencode($company['Name']).'&t=2&p=1&cn=1';
//        $res1 = json_decode(file_get_contents($url1),true);
//        $company['invention_count'] = $res1['cubePatentSearchResponse']['total_hits'];
//        M('Company')->where(array('id'=>I('get.id')))->save(
//            array('invention_count'=>$company['invention_count'])
//        );
//
//        $this->assign('res1',$res1);

//        if(time() - $company['patent_update'] > 3600*24*7){
//            $url1 = 'http://120.78.133.212:8000/get_patent_count/?keyword='.urlencode($company['Name']).'&t=2&p=1&cn=1';
//            $url2 = 'http://120.78.133.212:8000/get_patent_count/?keyword='.urlencode($company['Name']).'&t=2&p=1&cn=2';
//            $url3 = 'http://120.78.133.212:8000/get_patent_count/?keyword='.urlencode($company['Name']).'&t=2&p=1&cn=3';
//
//            $res1 = json_decode(file_get_contents($url1),true);
//            $res2 = json_decode(file_get_contents($url2),true);
//            $res3 = json_decode(file_get_contents($url3),true);
//
//            $company['invention_count'] = $res1['cubePatentSearchResponse']['total_hits'];
//            $company['appearance_count'] = $res2['cubePatentSearchResponse']['total_hits'];
//            $company['practical_count'] = $res3['cubePatentSearchResponse']['total_hits'];
//
//            M('Company')->where(array('id'=>I('get.id')))->save(
//                array(
//                    'invention_count'=>$company['invention_count'],
//                    'appearance_count'=>$company['appearance_count'],
//                    'practical_count'=>$company['practical_count'],
//                    'patent_update'=>time()
//                )
//            );
//            $this->assign('res1',$res1);
...
//        }
        ...
        $where1['KindCodeDesc'] = array('like','%发明%');
        $where1['company_id'] = I('get.id');
        $where2['KindCodeDesc'] = array('like','%实用%');
        $where2['company_id'] = I('get.id');
        $where3['KindCodeDesc'] = array('like','%外观%');
        $where3['company_id'] = I('get.id');
        $where4['company_id'] = I('get.id');

        $url = 'http://www.cnipsun.com/patent/search.do';
        $data = [
            'ascOrder'=>'',
            'cnSources'=>'fmzl_ft,fmsq_ft', //发明
            'field1'=>0,
            'field1Val'=>'',
            'field2'=>0,
            'field2Val'=>'',
        ...
        $result[0] = json_decode(httpPost($url,$data),true)['pageResult'];
        $data['cnSources'] = 'wgzl_ab';
        $result[1] = json_decode(httpPost($url,$data),true)['pageResult'];
        $data['cnSources'] = 'syxx_ft';
        $result[2] = json_decode(httpPost($url,$data),true)['pageResult'];


        $this->assign('result',$result);


//      $result[0] = M('Patent')->where($where1)->select();
//      $result[1] = M('Patent')->where($where2)->select();
//      $result[2] = M('Patent')->where($where3)->select();
//      $result[3] = M('Copyright')->where($where4)->select();

//        $url = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=发明授权&pageSize=50';
//        $url1 = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=发明公布&pageSize=50';

/*        $url2 = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=外观设计&pageSize=50';
        $url3 = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=%E5%AE%9E%E7%94%A8%E6%96%B0%E5%9E%8B&pageSize=50';


        $url = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=发明公布&pageSize=50';
        $url1 = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=发明授权&pageSize=50';

        $res =  json_decode(file_get_contents($url),true);
        $res2 =  json_decode(file_get_contents($url1),true);
        $this->assign('res',$res);
        $this->assign('res2',$res2);

        $result[1] = json_decode(file_get_contents($url2),true);
        $result[2] = json_decode(file_get_contents($url3),true);*/
...
//      //发明公布,发明授权
//      if($company['invention_count'] > 0){
//          if(count($result[0]) != $company['invention_count']){
//              $url = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=发明公布&pageSize=20';
//              $url2 = 'http://i.yjapi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=发明授权&pageSize=20';
//              $res =  json_decode(file_get_contents($url),true);
//              $res2 =  json_decode(file_get_contents($url2),true);
//              if($res['Status'] == 200){
...
//                      $temp = array();
//                      $temp['company_id'] = $company['id'];
//                      $temp['DocumentTypes'] = $value['DocumentTypes'];
//                      $temp['Agent'] = $value['Agent'];
//                      $temp['LegalStatusDate'] = strtotime($value['LegalStatusDate']);
//                      $temp['PrimaryExaminer'] = $value['PrimaryExaminer'];
//                      $temp['AssiantExaminer'] = $value['AssiantExaminer'];
//                      $temp['Cites'] = $value['Cites'];
//                      $temp['OtherReferences'] = $value['OtherReferences'];
//                      $temp['PatentImage'] = $value['PatentImage'];
//                      $temp['IPCList'] = implode(',',$value['IPCList']);
//                      $temp['ApplicationNumber'] = $value['ApplicationNumber'];
//                      $temp['ApplicationDate'] = strtotime($value['ApplicationDate']);
//                      $temp['PublicationNumber'] = $value['PublicationNumber'];
//                      $temp['PublicationDate'] = strtotime($value['PublicationDate']);
//                      $temp['LegalStatusDesc'] = $value['LegalStatusDesc'];
//                      $temp['Title'] = $value['Title'];
//                      $temp['Abstract'] = $value['Abstract'];
//                      $temp['Agency'] = implode(',',$value['Agency']);
//                      $temp['KindCodeDesc'] = $value['KindCodeDesc'];
//                      $temp['IPCDesc'] = implode(',',$value['IPCDesc']);
//                      $temp['InventorStringList'] = implode(',',$value['InventorStringList']);
//                      $temp['AssigneestringList'] = implode(',',$value['AssigneestringList']);
//                      if(!$this->patentModel->where(array('PublicationNumber'=>$value['PublicationNumber'],'company_id'=>$company['id']))->find()){
//                          $this->patentModel->add($temp);
//                      }
//                  }
//              }
//              if($res2['Status'] == 200){
//                  foreach($res2['Result'] as $key => $value){
//                      $temp = array();
//                      $temp['company_id'] = $company['id'];
//                      $temp['DocumentTypes'] = $value['DocumentTypes'];
//                      $temp['Agent'] = $value['Agent'];
...
//
//
//      if($company['practical_count'] > 0){
//          if(count($result[1]) != $company['practical_count']){
//              $url = 'http://i.yjApi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=实用新型&pageSize=50';
//              $res =  json_decode(file_get_contents($url),true);
//              if($res['Status'] == 200){
//                  foreach($res['Result'] as $key => $value){
//                      $temp = array();
...
//          }
//      }
//      if($company['appearance_count'] > 0){
//          if(count($result[2]) != $company['appearance_count']){
//              $url = 'http://i.yjApi.com/PatentV4/Search?key='.$this->ApiKey.'&searchkey='.$company['Name'].'&kindCodeDesc=外观设计&pageSize=50';
//              $res =  json_decode(file_get_contents($url),true);
//              if($res['Status'] == 200){
//                  foreach($res['Result'] as $key => $value){
//                      $temp = array();
...
//      }
    ...
    public function chanageFocus(){
        ...
        $data = array('company_id'=>$company_id,'user_id'=>session('user.user_id'));
        if($focus_status == 1){
            M('Focus')->add($data);
...
//              echo "</div>";
    ...
    public function searchCompany(){
        ...
        if(IS_POST){
            ...
            //             }
            //         }else{
            //             $companyList = getCompanyListByKeyword();
            //             if($companyList['Status'] == 200){
            //                 foreach($companyList['Result'] as $key => $value){
            //                     $value['StartDate'] = strtotime($value['StartDate']);
            //                     $map = array(
            ...
            // $this->assign('record',$this->unset_keyword(M('SearchRecord')->order(array('create_time'=>'desc'))->where(array('uid'=>session('user.user_id'),'type'=>7))->limit(10)->select()));
            // dump($result);
            $this->assign('result',$result);
            $this->assign('keyword',$keyword);
            $this->display();
